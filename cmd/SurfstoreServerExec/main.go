...
func startServer(hostAddr string, serviceType string, blockStoreAddrs []string) error {
	...
	surfMetaStore := surfstore.NewMetaStore(blockStoreAddrs)
	surfBlockStore := surfstore.NewBlockStore()
	//Step 2: Register rpc services
	if serviceType == "meta" {
		surfstore.RegisterMetaStoreServer(grpcServer, surfMetaStore)
	} else if serviceType == "block" {
		surfstore.RegisterBlockStoreServer(grpcServer, surfBlockStore)
	} else {
		surfstore.RegisterMetaStoreServer(grpcServer, surfMetaStore)
		surfstore.RegisterBlockStoreServer(grpcServer, surfBlockStore)
	}
